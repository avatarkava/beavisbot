...
exports.handler = function (data) {
    ...
    if (_.findWhere(room.users, {id: data.fid}).permission > 2 && theme) {

        if(theme == 'reset' || theme == 'clear') {
            theme = config.responses.theme;
        }

        db.run('REPLACE INTO SETTINGS (name, value, userid, timestamp) VALUES (?, ?, ?, CURRENT_TIMESTAMP)', ['theme', theme, data.fid],
            function(error) {
                if (error) {
