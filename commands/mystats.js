...
exports.handler = function(data) {
    ...
    db.all('SELECT userid FROM PLAYS GROUP BY userid ORDER BY sum(upvotes) DESC', function(err, rows) {
        rank = _.pluck(rows, 'userid').indexOf(data.uid.toString()) + 1;
        if (rank == 0) {
            bot.sendChat('You haven\'t played any songs in this room!');
        } else {
            db.get('SELECT count(*) as total, sum(upvotes) as up, avg(upvotes) as avg_up, sum(downvotes) as down, avg(downvotes) as avg_down, sum(snags) AS snag, avg(snags) AS avg_snag FROM PLAYS WHERE userid = ?', [data.uid], function(error, row) {
